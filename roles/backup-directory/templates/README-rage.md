...
Скрипт `backup-directory.sh` предназначен для создания резервных копий директорий с поддержкой различных режимов работы: упаковка файлов в архив или только шифрование без упаковки с использованием rage.

## Основные возможности

- **Упаковка файлов**: Создание tar-архивов с опциональным сжатием
- **Шифрование**: Rage шифрование архивов или отдельных файлов
...
- `--rage-recipient <key>` - Rage ключ получателя (по умолчанию: из конфигурации)
- `--no-encrypt` - Отключить rage шифрование
...
```
**Результат**: `backup_20241201_143022.tar.gz.age`
...
```
**Результат**: `backup_20241201_143022_encrypted/`
           ├── file1.txt.age
           ├── file2.log.age
           └── subdir/
               └── file3.conf.age
...
### 8. Работа с rage ключами
```bash
# Использование конкретного rage ключа
./backup-directory.sh \
  --source /opt/sensitive \
  --rage-recipient age1ql3z7hjy54pw3hyww5ayyfg7zqgvc7w3j2elw8zmrj2kg5sfn9aqmcac8p \
  --no-pack
```
...
- Опционально шифрует архив с помощью rage
- Подходит для полного резервного копирования

### Режим только шифрования
- Шифрует каждый файл отдельно без создания архива
- Создает структуру директорий с зашифрованными файлами
- Каждый файл получает расширение .age
...
|-------|--------|------------|------------------|
| Упаковка | Да | Да | `.tar.gz.age` |
| Упаковка | Да | Нет | `.tar.gz` |
| Упаковка | Нет | Да | `.tar.age` |
| Упаковка | Нет | Нет | `.tar` |
| Только шифрование | - | Да | `*_encrypted/` с файлами `.age` |

## Автоматическая очистка

Скрипт автоматически удаляет старые бэкапы:
- Для упакованных бэкапов: ищет файлы `prefix_*.tar.gz*`
- Для зашифрованных директорий: ищет папки `prefix_*_encrypted` и файлы `*.age`
...
- `rage` - для шифрования (если включено)
- Rage ключ получателя должен быть настроен в системе
